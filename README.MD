...
**KokoroDoki** is a real-time Text-to-Speech application that supports multiple languages and voices. It can run locally on your laptop and use either the CPU or leverage CUDA for GPU acceleration.

Powered by the [Kokoro-82M](https://huggingface.co/hexgrad/Kokoro-82M) model—an open-weight TTS model that provides high-quality, natural-sounding speech—it rivals larger models while remaining lightweight and highly responsive.
...
* **Audio/Voice Tools**: `portaudio`, `espeak-ng`
...
# Create a custom shortcut for seamless connection between client and daemon mode
```
...
### 1. 🖥️ Console Mode (Interactive Terminal)
...
### 3. 🛠️ Daemon Mode (Background Service)
...
# Speak from file and save audio
python src/main.py --file input.txt --output out.wav   
# Speak using every voice of a language
...
--TODO
<!-- KokoroDoki is released under the **MIT License**. -->
